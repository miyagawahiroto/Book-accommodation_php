...
class Dteam
{
    ...
    function get_hotelname($hotel_id)
    {
        $pdo = $this->get_pdo();
        $sql = "SELECT hotel_name FROM hotel_tbl WHERE hotel_id = $hotel_id";
        $ps = $pdo->prepare($sql);
        $ps->execute();
        $search = $ps->fetchAll();
        foreach ($search as $row) {
    ...
    function get_reservecose($room_id)
    {
        $pdo = $this->get_pdo();
        $sql = "SELECT service_title FROM room_tbl WHERE room_id = $room_id";
        $ps = $pdo->prepare($sql);
        $ps->execute();
        $search = $ps->fetchAll();
        foreach ($search as $row) {
    ...
    function get_chktime($hotel_id)
    {
        $pdo = $this->get_pdo();
        $sql = "SELECT checkin_time FROM hotel_tbl WHERE hotel_id = $hotel_id";
        $ps = $pdo->prepare($sql);
        $ps->execute();
        $search = $ps->fetchAll();
        foreach ($search as $row) {
    ...
    function get_reservelist($user_id)
    {
        $pdo = $this->get_pdo();
        $sql = "SELECT * FROM reserve_tbl WHERE user_id = ?";
        $ps = $pdo->prepare($sql);
        $ps->bindValue(1, $user_id, PDO::PARAM_STR);
        $ps->execute();
        $search = $ps->fetchAll();
        ...
        foreach ($search as $row) {
            ...
            $hotel_name = $this->get_hotelname($row['hotel_id']);
            $user_name = $this->get_username($row['hotel_id']);
            $cose_name = $this->get_reservecose($row['room_id']);
            $chk_time = $this->get_chktime($row['hotel_id']);
            array_push($data, array('reserve_id' => $row['reserve_id'], 'hotel_id' => $row['hotel_id'], 'room_id' => $row['room_id'], 'checkin_date' => $row['checkin_date'], 'checkout_date' => $row['checkout_date'], 'reservation_number' => $row['reservation_number'], 'img' => $photo, 'hotelname' => $hotel_name, 'username' => $user_name, 'cosename' => $cose_name, 'checkin_time' => $chk_time));
        }
        return $data;
    }
    ...
    {
        $pdo = $this->get_pdo();
        $sql = "SELECT * FROM room_tbl WHERE hotel_id = ?";
        $ps = $pdo->prepare($sql);
        $ps->bindValue(1, $hotel_id, PDO::PARAM_STR);
        $ps->execute();
        $search = $ps->fetchAll();
    ...
    {
        $pdo = $this->get_pdo();
        $sql = "DELETE FROM reserve_tbl WHERE reserve_id = ?";
        $ps = $pdo->prepare($sql);
